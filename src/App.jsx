  ...
  return (
    ...
    >
        ...
        {slides.map((slide, i) => (
          ...
          >
            ...
            <div className={`rounded-full transition-all duration-300 ${
              i === current ? "w-1.5 h-6 bg-white" : "w-1.5 h-1.5 bg-white/30 hover:bg-white/60"
            }`} />
          </button>
        ))}
      </div>
      ...
      <div className="fixed top-5 left-6 z-50 flex items-center gap-3">
        <img src="/WEM_Cocoa_Logo.svg" alt="West Edmonton Mall" className="h-7 w-auto brightness-0 invert opacity-80" />
      ...
      {/* PREV ARROW */}
      {current > 0 && (
        <button onClick={goPrev} className="fixed bottom-16 left-1/2 -translate-x-1/2 z-50 text-white/30 hover:text-white transition duration-300 rotate-180">
      ...
      {current < slides.length - 1 && (
        <button onClick={goNext} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 text-white/30 hover:text-white transition duration-300">
          <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
          </svg>
        </button>
